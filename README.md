...
Every user mode program has an associated page table which maps its virtual address space to the machine's physical address space. The base address of the Page Table (of the user mode program currently in execution) is stored in the __Page Table Base Register (PTBR)__ and the number of entries in this Page Table must be stored in the Page Table Length Register (PTLR). The page tables must be set up in the **privileged mode**.
